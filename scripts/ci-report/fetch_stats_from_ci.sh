...
while read -r run; do
	...
	# https://github.com/onchainengineering/hmi-computer/issues/6676
	...
	while read -r job; do
		...
		if [[ ! -f "${job_log}" ]]; then
			...
			if [[ ${USE_GH_RUN_VIEW_LOGS} -eq 0 ]]; then
				...
				# https://github.com/onchainengineering/hmi-computer/commit/${head_sha}/checks/${job_database_id}/logs/${job_step_number}
